...
class TestDecodePacketType(object):
    NOMINAL_CASES = [
...
def test_packet_type_encoding_endtoend(packet_type):
    """Tests end-to-end encoding and decoding of packet types.
    """
    assert protocol.PacketType.decode(protocol.PacketType.encode(packet_type)) == (packet_type, 2)


class TestEncodeTransferMode(object):
    NOMINAL_CASES = [
...
class TestDecodeTransferMode(object):
    NOMINAL_CASES = [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Unfortunately, constantly doesn't have good support for aliases, and so ASCII != NETASCII
        # (etc.) even if that's my intention.
...
def test_transfer_mode_encoding_endtoend(transfer_mode, exp_output):
    """Tests end-to-end encoding and decoding of transfer mode.
    """
    assert protocol.TransferMode.decode(protocol.TransferMode.encode(transfer_mode)) == exp_output


class TestEncodeErrorCode(object):
    NOMINAL_CASES = [
...
class TestDecodeErrorCode(object):
    NOMINAL_CASES = [
...
def test_error_code_encoding_endtoend(error_code):
...
class TestOptionMisc(object):
    ...
    def test_repr_coverage_only(self):
        """Runs the Option.__repr__() function just to ensure that it doesn't raise exceptions or
...
def test_option_encoding_endtoend(name, value):
